...
func part1(input string) int {
	...
	for i := 0; i < 10; i++ {
		...
		for k, elf := range elfs {
			...
			if exists {
				...
				if len(proposal) > 0 {
					...
					proposalKey := createKey(proposal[0], proposal[1])
					_, pe := proposals[proposalKey]
					if !pe {
						proposals[proposalKey] = 1
					} else {
						proposals[proposalKey] += 1
					}
				} else {
		...
		for _, elf := range elfs {
			...
			_, elfExists := newElfs[createKey(elf.y, elf.x)]
			if elfExists {
				panic("WTF")
			}
